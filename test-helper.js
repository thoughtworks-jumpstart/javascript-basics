    ...
    test: function (actualValue, expectedValue) {
      if (Array.isArray(expectedValue)) {
        if (this.checkArrayEquality(expectedValue, actualValue) === false) {
          console.log('\x1b[31m',`  \u00D7 TEST FAILED: expected [${expectedValue}] but got [${actualValue}] instead`)
        } else if (this.checkArrayEquality(expectedValue, actualValue) === true) {
          console.log('\x1b[32m', `  \u2713 TEST PASSED!: expected [${expectedValue}] and got [${actualValue}]`)
        }
      } else {
